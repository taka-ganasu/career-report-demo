 ...
 * report.md を生成
 */
export function generateReportMarkdown(userId: string, q5: Q5Result): string {
 ...
 * report.md を出力
 */
export function writeReportMarkdown(outDir: string, userId: string, q5: Q5Result): string {
  const filePath = `${outDir}/${userId}.report.md`;
  const md = generateReportMarkdown(userId, q5);
